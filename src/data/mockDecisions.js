...
export const SUGGESTIONS = SUGGESTIONS_BY_STORE.S001

export function getDecisionsMock(storeId) {
  const sid = storeId || 'S001'
  const list = SUGGESTIONS_BY_STORE[sid] || SUGGESTIONS_BY_STORE.S001
  return {
    date: new Date().toISOString().slice(0, 10),
    restock: list.filter(s => s.action === 'Restock'),
    promotion: list.filter(s => s.action === 'Promotion'),
    storeId: sid,
  }
}
