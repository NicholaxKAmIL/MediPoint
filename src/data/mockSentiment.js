...
const SOURCES = {
  ...
  GovNotice: [
    ...
    { title: '福建 CDC 流感周报：本周 ILI% 上升至 8.4%', content: '流感样病例就诊比例明显上升，以 A(H3N2) 为主。', tags: ['福建', 'CDC', '流感周报'], sentiment: 'concern', url: 'https://www.fjcdc.com.cn/' },
    ...
    { title: 'NMPA 通报：某批次铝碳酸镁片三级召回', content: '涉事批次溶出度不符合标准，可能影响疗效。', tags: ['NMPA', '召回', '胃药'], sentiment: 'negative', url: 'https://www.nmpa.gov.cn/' },
    { title: '福建 CDC 老年人流感疫苗免费接种通知', content: '福州、厦门、泉州三地 65 岁以上老人可到社区免费接种四价流感疫苗。', tags: ['福建', '疫苗', '老人'], sentiment: 'positive', url: 'https://www.fjcdc.com.cn/' },
  ],
}
...
export function getSentimentMock() {
  const items = []
  ...
  return { items, keyword_trend: KEYWORD_TREND, seven_day_series: SEVEN_DAY }
}
